...
def LogisticRegression():
    ...
    print('在训练集上的准确度为%f%%' % np.mean(np.float64(p == y) * 100))  # 与真实值比较，p==y返回True，转化为float
...
def plotDecisionBoundary(theta, X, y):
    ...
    z = np.transpose(z)
    plt.contour(u, v, z, [0, 0.01], linewidths=2.0)  # 画等高线，范围在[0,0.01]，即近似为决策边界
    plt.show()
